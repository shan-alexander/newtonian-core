...
## This crate (read first)

You are in **`newtonian-core`** (repo dir `newton-core`), the **policy kernel** crate (`0.1.0`): ports, `step_entity`, optional `fold` / `machine`, optional `exec` module. You are **not** in `newton-machine` (the chart). Do not open a `newton-exec` crate — that was rejected in ADR 0015. Hosts with a pulse already should ignore `::exec`.

A Newton program is an Executive running one or more Newton machines against a belief store and a gateway.

**Before large edits:** `rustbrain context "what is a newtonian program"`. Then read `README.md`, `docs/goals/README.md`, `docs/adr/0002-family-is-three-crates.md`, `docs/adr/0010-not-a-domain-crate.md`.

Hard refusals: no `run()` here, no sockets, no trading types, no YAML chart topology, no folding the Executive into the machine, do not call this “the behavior layer.”
