...
`newtonian-core` is the **policy kernel** (`0.1.0` ships ports + kernel + Fold). `newton-machine` is UCA truth. There is no `newton-exec` crate. Orient with `rustbrain context "policy kernel"` then `rustbrain context "kernel sits beside the chart"` before claiming what this crate owns. `step_entity` does not call `apply`. Do not invent ADR history. Do not add domain vocabulary (orders, bars, ROS, DOM). Do not put a sleeve table inside Newton `update`. Exact key; unauthored → none.
